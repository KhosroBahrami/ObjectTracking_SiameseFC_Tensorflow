...
from training.train_utils import construct_gt_score_maps, load_mat_model
...
class SiameseModel:
  ...
  def build_inputs(self):
    ...
    if self.mode in ['train', 'validation']:
      with tf.device("/cpu:0"):  
        self.dataloader = DataSet(self.mode, self.is_training())
      ...
      self.examplar_feed = tf.placeholder(shape=[None, None, None, 3], dtype=tf.uint8, name='examplar_input')
      self.instance_feed = tf.placeholder(shape=[None, None, None, 3], dtype=tf.uint8, name='instance_input')
  ...
  def build_image_embeddings(self, reuse=False):
    ...
    """
    arg_scope = convolutional_alexnet_arg_scope(trainable=FLAGS.model_train_embedding,
  ...
  def build_detection(self, reuse=False):
    with tf.variable_scope('detection', reuse=reuse):
      ...
      bias = tf.get_variable('biases', [1],
                             ...
                             trainable=FLAGS.model_adjust_train_bias)
  ...
  def build_loss(self):
    ...
    gt = construct_gt_score_maps(response_size, self.mode) 
                                 
    with tf.name_scope('Loss'):
      loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=response, labels=gt)
      ...
      with tf.name_scope('Balance_weights'):
        ...
        w_pos = 0.5 / n_pos
        w_neg = 0.5 / n_neg
        class_weights = tf.where(tf.equal(gt, 1), w_pos * tf.ones_like(gt), tf.ones_like(gt))
        class_weights = tf.where(tf.equal(gt, 0), w_neg * tf.ones_like(gt), class_weights)
  ...
  def setup_embedding_initializer(self):
    ...
    if FLAGS.model_checkpoint_file:
      initialize = load_mat_model(FLAGS.model_checkpoint_file, 'alexnet/', 'detection/')

      def restore_fn(sess):
        tf.logging.info("Restoring embedding variables from checkpoint file %s", FLAGS.model_checkpoint_file)
        sess.run([initialize])
