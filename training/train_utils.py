...
def construct_gt_score_maps(response_size, mode):
  ...
  with tf.name_scope('construct_gt'):
    ...
    def _logistic_label(X, Y, rPos, rNeg):
      ...
      Z = tf.where(dist_to_center <= rPos, tf.ones_like(X),
                   tf.where(dist_to_center < rNeg, 0.5 * tf.ones_like(X), tf.zeros_like(X)))
      return Z

    #rPos = gt_config['rPos'] / FLAGS.model_stride
    rPos = FLAGS.train_rPos / FLAGS.model_stride
    #rNeg = gt_config['rNeg'] / stride
    rNeg = FLAGS.train_rNeg / FLAGS.model_stride
